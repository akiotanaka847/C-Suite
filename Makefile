...
dev:
	@echo "Starting C-Suite..."
	@cd packages/core && uv run uvicorn csuite.api.main:app --reload --port 8000 &
...
discord:
	cd packages/core && uv run python -m csuite.integrations.discord_bot

seed-knowledge:
	cd packages/core && uv run python -c "from csuite.knowledge.loader import seed_builtin_knowledge; import asyncio; asyncio.run(seed_builtin_knowledge())"
