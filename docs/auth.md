...
C-Suite is gated behind Google sign-in plus an email allow-list. This doc explains what's protected, how, and how to operate it (add/remove users, rotate secrets, debug failures).
...
Browser ──► csuite-ui-dev.fly.dev ──► (middleware: session check)
                ...
                └── has session ──► /api/backend/[...path] (proxy)
                                        │ stamps x-api-key
                                        ▼
                                 csuite-api-dev.fly.dev (FastAPI)
...
See [`_UNAUTHENTICATED_PATHS`](../packages/core/csuite/api/main.py) — any new webhook from an external service must be added here.
...
3. **APIs & Services → Clients → + Create Client → Web application**:
   - **Authorized JavaScript origins**: `http://localhost:3000`, `https://csuite-ui-dev.fly.dev`
   - **Authorized redirect URIs**: `http://localhost:3000/api/auth/callback/google`, `https://csuite-ui-dev.fly.dev/api/auth/callback/google`
...
> **Production fails closed.** If `BACKEND_SHARED_SECRET` is unset and `FLY_APP_NAME` is set (i.e. running on Fly), [api/main.py](../packages/core/csuite/api/main.py) raises `RuntimeError` at startup rather than serve traffic without auth.
...
```bash
flyctl secrets set -a csuite-ui-dev ALLOWED_EMAILS="alice@x.com,bob@y.com,carol@z.com"
```
...
flyctl secrets set -a csuite-ui-dev  BACKEND_SHARED_SECRET="$NEW"
flyctl secrets set -a csuite-api-dev BACKEND_SHARED_SECRET="$NEW"
```

There's a brief window during the rolling restart where one app has the new value and the other has the old one. Calls during that window will 401. If that matters, take the UI down first (`flyctl scale count 0 -a csuite-ui-dev`), rotate both, then scale back up.
...
```bash
flyctl secrets set -a csuite-ui-dev AUTH_SECRET="$(openssl rand -base64 32)"
```
...
| Sign-in works but the chat stays empty | Backend is auth'd but `ANTHROPIC_API_KEY` is missing on `csuite-api-dev`. `flyctl logs -a csuite-api-dev` will show the error |

### Useful commands

```bash
# What secrets are set on each app (digests only, not values)
flyctl secrets list -a csuite-ui-dev
flyctl secrets list -a csuite-api-dev
...
curl -sv https://csuite-api-dev.fly.dev/health           # should be 200
curl -sv https://csuite-api-dev.fly.dev/sessions          # should be 401
curl -sv -H "x-api-key: $SHARED" https://csuite-api-dev.fly.dev/sessions  # should be 200
```
...
If/when per-user data isolation matters (e.g. private sessions per teammate), the change is non-trivial: habría que añadir `user_id` a la tabla de sesiones.
