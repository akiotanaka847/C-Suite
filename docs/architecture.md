...
Slack Bolt app in socket mode. Listens for `@CSuite` mentions and direct messages. Calls `Executive.chat()` synchronously via `asyncio.run()`. Runs as a separate process alongside FastAPI.
...
1. Create `packages/core/csuite/agents/your_agent.py`:
