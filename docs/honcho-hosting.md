...
```
csuite-api-dev (Fly app)
    │
    │ POST/GET via httpx → http://csuite-honcho-dev.flycast:8000
    ▼
csuite-honcho-dev (Fly app, this runbook)
    ...
    │                     embed.process.csuite-honcho-dev.internal:8001
...
```bash
flyctl apps create csuite-honcho-dev --org <your-org>
```
...
# csuite-honcho-dev.
flyctl postgres attach csuite-honcho-pg -a csuite-honcho-dev

# Enable pgvector. Honcho's migrations expect the extension to exist.
flyctl postgres connect -a csuite-honcho-pg \
...
```bash
RAW_URL=$(flyctl ssh console -a csuite-honcho-dev -C 'printenv DATABASE_URL')
SQLA_URL=$(echo "$RAW_URL" | sed 's|^postgres://|postgresql+psycopg://|')
flyctl secrets set -a csuite-honcho-dev DB_CONNECTION_URI="$SQLA_URL"
```
...
```bash
flyctl secrets set -a csuite-honcho-dev \
    LLM_OPENAI_API_KEY="$OPENROUTER_API_KEY" \
    AUTH_JWT_SECRET="$(openssl rand -hex 32)" \
    EMBEDDING_MODEL_CONFIG__OVERRIDES__BASE_URL="http://embed.process.csuite-honcho-dev.internal:8001/v1" \
...
```bash
flyctl status -a csuite-honcho-dev
...
```bash
flyctl ssh console -a csuite-honcho-dev --process-group api \
...
to `csuite-api-dev`'s Fly secrets can take over the Honcho deployment.
...
the admin JWT and a body limiting scope to the `csuite` workspace
(see Honcho's API docs); never put an admin JWT in
`csuite-api-prod`'s secrets. If a dev admin JWT leaks, rotate
...
flyctl secrets set -a csuite-api-dev \
    HONCHO_ENABLED=true \
    HONCHO_API_KEY=hch-... \
    HONCHO_BASE_URL=http://csuite-honcho-dev.flycast:8000 \
...
```bash
flyctl logs -a csuite-honcho-dev | grep -i deriver
```
...
```bash
flyctl logs -a csuite-honcho-dev                              # tail both processes
flyctl logs -a csuite-honcho-dev --process-group api          # api only
flyctl logs -a csuite-honcho-dev --process-group deriver      # deriver only
```
...
DB_URL=$(flyctl ssh console -a csuite-honcho-dev --process-group api \
...
flyctl proxy 5432 -a csuite-honcho-pg &   # tunnels Fly PG to localhost:5432
PROXY_PID=$!
...
| 1 | `BAAI/bge-base-en-v1.5` (768-dim) | +300MB image, +400MB RAM | `flyctl secrets set -a csuite-honcho-dev EMBED_MODEL=BAAI/bge-base-en-v1.5` (then rebuild + redeploy — fastembed needs the model in its cache, which is baked at build time) |
| 2 | `BAAI/bge-large-en-v1.5` (1024-dim) | +1.2GB image, +1.2GB RAM | same pattern; bump the `embed` VM to 2GB in `fly.honcho.toml` first |
| 3 | OpenRouter `openai/text-embedding-3-small` (1536-dim) | network hop, ~$0.02/M tokens | `flyctl secrets set -a csuite-honcho-dev EMBEDDING_MODEL_CONFIG__OVERRIDES__BASE_URL=https://openrouter.ai/api/v1 EMBEDDING_MODEL_CONFIG__OVERRIDES__API_KEY="$OPENROUTER_API_KEY" EMBEDDING_VECTOR_DIMENSIONS=1536` AND change `[embedding.model_config].model` in `config.toml` to `openai/text-embedding-3-small`. The vector-dim change triggers `configure_embeddings.py --yes` (run by `release_command`) to ALTER the pgvector column from 384 to 1536 — destructive on existing embeddings; back up first. |
...
flyctl secrets set -a csuite-honcho-dev AUTH_JWT_SECRET="$NEW_SECRET"
...
1. Generate a Fly API token scoped to `csuite-honcho-dev`:
   ```bash
   flyctl tokens create deploy -a csuite-honcho-dev
   ```
...
- `csuite-honcho-prod` app.
- `csuite-honcho-prod-pg` Postgres cluster (consider larger VM + larger
