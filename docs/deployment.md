...
|---|---|---|---|
| `csuite-api-dev` | [docker/Dockerfile](../docker/Dockerfile) — Python 3.11 + FastAPI + scheduler | `https://csuite-api-dev.fly.dev` | Persistent volume `executive_data` mounted at `/data` |
| `csuite-ui-dev` | [docker/Dockerfile.ui](../docker/Dockerfile.ui) — Next.js 15 standalone build | `https://csuite-ui-dev.fly.dev` | Stateless |
...
The scheduler in [packages/core/csuite/scheduler](../packages/core/csuite/scheduler) claims due actions with `UPDATE … RETURNING`. Two machines running the same scheduler would double-fire (and double-bill the Anthropic API). The API app is configured to keep this from happening:
...
On first boot, the volume is empty — ChromaDB rebuilds the built-in knowledge index from files shipped inside the Python package at `csuite/knowledge/builtin/`. The episodic SQLite is created on demand. The company profile stays empty until you run the onboarding wizard against the dev URL.
...
```
flyctl apps create csuite-api-dev
flyctl apps create csuite-ui-dev
```

### 3. Create the volume

```
flyctl volumes create executive_data --region iad --size 1 -a csuite-api-dev
```
...
```
flyctl tokens create deploy -a csuite-api-dev -x 999999h
flyctl tokens create deploy -a csuite-ui-dev  -x 999999h
```
...
| Secret name | Value |
|---|---|
| `FLY_API_TOKEN_API` | token from the `csuite-api-dev` command |
| `FLY_API_TOKEN_UI`  | token from the `csuite-ui-dev` command |
...
- **Telegram**: `curl -F "url=https://csuite-api-dev.fly.dev/webhook/telegram" "https://api.telegram.org/bot<DEV_TOKEN>/setWebhook"`
- **Google OAuth**: in the GCP console, add `https://csuite-api-dev.fly.dev/oauth/google/callback` as an authorized redirect URI on the OAuth client.
...
|---|---|---|---|
| `csuite-api-qa` | [fly.api.qa.toml](../fly.api.qa.toml) — same image as dev | `https://csuite-api-qa.fly.dev` | Persistent volume `executive_data` at `/data` |
| `csuite-ui-qa` | [fly.ui.qa.toml](../fly.ui.qa.toml) — `BACKEND_BASE_URL` points at the QA API | `https://csuite-ui-qa.fly.dev` | Stateless |
...
```
flyctl apps create csuite-api-qa
flyctl apps create csuite-ui-qa
flyctl volumes create executive_data --region iad --size 1 -a csuite-api-qa
flyctl tokens create deploy -a csuite-api-qa -x 999999h   # → FLY_API_TOKEN_API_QA
flyctl tokens create deploy -a csuite-ui-qa  -x 999999h   # → FLY_API_TOKEN_UI_QA
```
...
flyctl secrets set -a csuite-api-qa \
  ANTHROPIC_API_KEY=sk-ant-... \
  BACKEND_SHARED_SECRET=$(openssl rand -hex 32) \
  BACKEND_ALLOWED_ORIGINS=https://csuite-ui-qa.fly.dev \
  HONCHO_ENABLED=true HONCHO_API_KEY=... HONCHO_BASE_URL=... \
  HONCHO_WORKSPACE_ID=csuite-qa \
...
flyctl secrets set -a csuite-ui-qa \
  AUTH_SECRET=$(openssl rand -base64 32) \
  AUTH_GOOGLE_ID=... AUTH_GOOGLE_SECRET=... ALLOWED_EMAILS=... \
  AUTH_TRUST_HOST=true AUTH_URL=https://csuite-ui-qa.fly.dev \
  BACKEND_SHARED_SECRET=<same value as the API app>
```

Then add `https://csuite-ui-qa.fly.dev/api/auth/callback/google` as an authorized redirect URI on the OAuth client, create separate QA bot apps for any channels QA should run (so QA and dev never double-reply), and complete the onboarding wizard against the QA URL — QA starts with an empty volume by design.
...
```
flyctl ssh console -a csuite-api-dev -C "mkdir -p /data/google_credentials"
flyctl ssh sftp shell -a csuite-api-dev
...
```
flyctl apps restart csuite-api-dev   # gateway reads the config + secrets at startup
```

Confirm: `flyctl logs -a csuite-api-dev` shows `MCPGateway started`, and a
...
If a previous setup created `csuite-gworkspace-dev`, retire it after the
co-located path works: revert the `google_workspace` entry in
`/data/company/mcp_servers.json` to the launcher form, then
`flyctl apps destroy csuite-gworkspace-dev` (this also removes its
...
> The commands below use the dev app names. For QA, swap `-a csuite-api-dev` → `-a csuite-api-qa` (and likewise for the UI app).

### Tail logs

```
flyctl logs -a csuite-api-dev
flyctl logs -a csuite-ui-dev
```

### Status

```
flyctl status -a csuite-api-dev      # machine state, volume attach, last release
flyctl releases -a csuite-api-dev    # release history
```

### Smoke checks

```
curl https://csuite-api-dev.fly.dev/health
# {"status":"ok","builtin_knowledge_chunks":...,"version":"0.1.0"}

curl https://csuite-ui-dev.fly.dev/api/backend/health
...
```
flyctl secrets set -a csuite-api-dev SOME_KEY=newvalue
```
...
```
flyctl releases -a csuite-api-dev
flyctl release rollback <version> -a csuite-api-dev
```

### Roll the volume forward / get a shell on it

```
flyctl ssh console -a csuite-api-dev
# inside: ls /data
```
...
| Deploy step says success, `/health` times out | App crashed during boot — usually a missing env var | `flyctl logs -a csuite-api-dev`; look for a Pydantic `ValidationError` and add the missing secret |
...
| Two scheduled actions firing at the same time | Two API machines running | `flyctl status -a csuite-api-dev` — if more than one machine, `flyctl scale count 1 -a csuite-api-dev` |
