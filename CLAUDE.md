...
1. Crear `packages/core/csuite/agents/tu_agente.py`:
...
`packages/core/csuite/architecture/prebuilt/<section_id>.json` — un archivo por
...
| Módulo top-level nuevo en `packages/core/csuite/` | `SectionSpec` en `sections.py` + entrada en `packages/ui/src/app/architecture/page.tsx` (los IDs deben coincidir) + `prebuilt/<id>.json` nuevo |
...
(`csuite-api-dev`, `csuite-ui-dev`). Antes de desplegar hay que renombrarlas:
...
| `csuite-api` | Golpear los endpoints del backend con curl. |
