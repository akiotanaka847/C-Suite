...
**On this repo, skip discovery — use the commands in `## Project: C-Suite` → "Verification commands".** The generic procedure below applies only to other codebases:
...
- Lint + type check: `make lint`
  (= `cd packages/core && uv run ruff check csuite/ && uv run mypy csuite/`)
- Tests: `make test`
...
`packages/core/csuite/architecture/architecture-facts.yaml`; the topic→key
...
`packages/core/csuite/`. **A change to what an existing key already
...
```bash
grep -nE '^[a-zA-Z_]+:' packages/core/csuite/architecture/architecture-facts.yaml
```
...
TOUCHED_TOPIC=$(echo "$CHANGED" | grep -E 'csuite/(integrations|scheduler)/|orchestrator/router\.py|prompts/cache_manager\.py' || true)
...
```bash
git --no-pager diff --staged -- packages/core/csuite/prompts/cache_manager.py \
  packages/core/csuite/prompts/executive_persona.py | \
...
- **Eval scenarios for new agents / prompt changes**
  ...
  `packages/core/csuite/agents/` or modifies
...
- **Tests for new behavior** (`check_name='tests-present'`, WARN — Medium
  ...
  `packages/core/csuite/` with no corresponding change under
