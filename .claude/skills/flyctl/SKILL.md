---
name: flyctl
description: Operate the two Fly.io apps for C-Suite (csuite-api-dev, csuite-ui-dev). Use this skill whenever the user asks to tail logs, SSH for SQLite inspection, list/set/unset secrets, check deploy status, restart machines, deploy, or rotate AUTH_SECRET. Covers the exact commands, SQLite paths, and known gotchas for this project. Tiered safety: read-only runs freely; destructive needs explicit user confirmation.
---

# flyctl

Quick-reference cheatsheet for operating C-Suite's two Fly.io apps. Pair skill: `csuite-api` (HTTP curl recipes against the FastAPI surface).

## Apps

| App | Stack | Key secrets | URL |
|---|---|---|---|
| `csuite-api-dev` | FastAPI (Python, port 8000) | `ANTHROPIC_API_KEY`, `HONCHO_API_KEY`, `HONCHO_BASE_URL` (optional), `BACKEND_SHARED_SECRET` | https://csuite-api-dev.fly.dev |
| `csuite-ui-dev` | Next.js 15 (NextAuth + Google OAuth) | `AUTH_SECRET`, `AUTH_GOOGLE_ID/SECRET`, `BACKEND_SHARED_SECRET`, `BACKEND_URL` | https://csuite-ui-dev.fly.dev |
...
```bash
flyctl status -a csuite-api-dev
flyctl status -a csuite-ui-dev

flyctl releases -a csuite-api-dev | head -10

# Live tail
flyctl logs -a csuite-api-dev

# One-shot capture (recent only, exits)
flyctl logs -a csuite-api-dev --no-tail
...
flyctl ssh console -a csuite-api-dev -C 'sh -c "sqlite3 /data/episodic_memory.db \"SELECT ts, json_extract(details_json,'\''$.op'\''), json_extract(details_json,'\''$.outcome'\'') FROM audit_log WHERE event_type='\''peer_memory'\'' ORDER BY id DESC LIMIT 10;\""'

# People roster
flyctl ssh console -a csuite-api-dev -C 'sh -c "sqlite3 /data/episodic_memory.db \"SELECT id, name, email, archived_at FROM people ORDER BY id;\""'

# Pending scheduled actions
flyctl ssh console -a csuite-api-dev -C 'sh -c "sqlite3 /data/episodic_memory.db \"SELECT id, kind, status, run_at FROM scheduled_actions WHERE status='\''pending'\'' ORDER BY run_at LIMIT 10;\""'

# Active Honcho workspace
flyctl ssh console -a csuite-api-dev -C 'sh -c "cat /data/_user_backup/.honcho_active_workspace.json 2>/dev/null || echo null"'
```
...
flyctl secrets set AUTH_SECRET="$(openssl rand -base64 33)" -a csuite-ui-dev

# Unset a secret (also redeploys)
flyctl secrets unset HONCHO_BASE_URL -a csuite-api-dev
```
