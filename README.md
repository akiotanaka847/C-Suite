...
[![CI](https://github.com/akiotanaka847/C-Suite/actions/workflows/ci.yml/badge.svg)](https://github.com/akiotanaka847/C-Suite/actions/workflows/ci.yml)
...
C-Suite provides a single coherent executive voice backed by eight specialist AI agents:
...
```bash
# Clone the repo
git clone https://github.com/akiotanaka847/C-Suite.git
cd C-Suite
...
uvicorn csuite.api.main:app --reload --port 8000
...
The machine restarts and the bot starts on the next lifespan boot. To disable in prod: `flyctl secrets unset -a csuite-api-dev DISCORD_BOT_TOKEN`.
...
| **Slack** | Mention `@CSuite` or DM the app |
...
|---|---|---|---|
| **dev** | push/merge to `main` (continuous) | `.github/workflows/deploy.yml` | `csuite-api-dev`, `csuite-ui-dev` |
| **qa** | push/merge to `qa` (deliberate promotion) | `.github/workflows/deploy-qa.yml` | `csuite-api-qa`, `csuite-ui-qa` |
...
| App | Purpose | State |
|-----|---------|-------|
| `csuite-api-{dev,qa}` | FastAPI + scheduler | Persistent volume `executive_data` at `/data` |
| `csuite-ui-{dev,qa}` | Next.js 15 | Stateless |
| `csuite-honcho-dev` | Honcho per-person memory (optional) | Postgres-backed |
...
| Secret | App | Used by |
|---|---|---|
| `FLY_API_TOKEN_API` | `csuite-api-dev` | dev |
| `FLY_API_TOKEN_UI` | `csuite-ui-dev` | dev |
| `FLY_API_TOKEN_HONCHO` | `csuite-honcho-dev` | dev (honcho job) |
| `FLY_API_TOKEN_API_QA` | `csuite-api-qa` | qa |
| `FLY_API_TOKEN_UI_QA` | `csuite-ui-qa` | qa |
...
flyctl secrets set -a csuite-api-dev ANTHROPIC_API_KEY=sk-ant-...
...
C-Suite can run against any **OpenAI-compatible** local server — Ollama,
...
1. Create `packages/core/csuite/agents/your_agent.py` extending `BaseAgent`
2. Add a system prompt constant in `packages/core/csuite/prompts/domain_prompts.py`
3. Register in `packages/core/csuite/orchestrator/router.py` — add to `SPECIALIST_REGISTRY` and the `specialist` enum in `SPECIALIST_TOOLS`
4. Add domain alias to `DOMAIN_ALIASES` in `packages/core/csuite/knowledge/retriever.py`
