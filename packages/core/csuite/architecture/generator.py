...
from csuite.architecture.cache import SectionContent, utc_now_iso
from csuite.architecture.facts import FactsBundle
from csuite.architecture.sections import SectionSpec
...
_SYSTEM_PROMPT = """\
You are a senior technical writer documenting the **C-Suite** system — a multi-agent AI executive built on the Anthropic Claude API.

Your job: produce ONE architecture-page section. The page lives at /architecture in the C-Suite web UI. You will be told which section to write, given the section's spec, and given a bundle of authoritative facts about the live system. **Ground every claim in the provided facts. Do not invent components, files, or behaviours.** If a fact you would need is missing, write around it.
...
- When you name a code symbol, render it as inline `code`. Use full module paths when helpful: `csuite.orchestrator.router.route_parallel`.
