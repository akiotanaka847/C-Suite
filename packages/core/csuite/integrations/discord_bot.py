"""Discord bot integration for C-Suite.
...
API. `run_discord_bot()` / `python -m csuite.integrations.discord_bot`
...
_GREETING_ADDRESSEES: frozenset[str] = frozenset({
    "there", "exec", "executive", "oe", "c-suite",
...
async def _should_respond_in_thread(
    ...
    """
    from csuite.integrations.response_gate import should_respond
...
async def _generate_thread_title(user_msg: str, assistant_msg: str) -> str | None:
    ...
    """
    from csuite.utils.session_title import generate_session_title
...
def _schedule_thread_rename(
    ...
    """
    from csuite.memory.session_store import update_session_title
...
async def _handle_message(
    ...
    from csuite.people.store import find_person_by_discord_id
    sender_person = (
    ...
    if discord_user_id:
        try:
            from csuite.people.store import find_person_by_discord_id
            from csuite.workflows.inbound_resolver import resolve_inbound_message
            from csuite.workflows.resumer import apply_resolution
    ...
    try:
        from csuite.alerts.models import AlertEvent
        from csuite.alerts.pipeline import schedule_evaluation
    ...
    from csuite.orchestrator.mcp_gateway import get_active_gateway
    ...
    async with _session_lock(session_id):
        try:
            ...
            if is_dm and discord_user_id:
                from csuite.integrations.inbound_hydration import (
...
def create_discord_bot():
    ...
    async def slash_today(interaction: discord.Interaction) -> None:
        ...
        await interaction.response.defer(thinking=True)
        try:
            from csuite.api.routes.today import _build_today
    ...
    async def on_message(message: discord.Message) -> None:
        ...
        if message.attachments:
            try:
                from csuite.integrations.attachments import (
                    AttachmentItem,
                    process_attachments,
                )
