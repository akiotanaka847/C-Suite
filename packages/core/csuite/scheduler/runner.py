...
if TYPE_CHECKING:
    from csuite.staff_onboarding.models import OnboardingPlan

from csuite.memory.episodic import (
    ScheduledAction,
    claim_due_actions,
    mark_action_done,
...
from csuite.orchestrator.mcp_gateway import MCPGateway
...
def _company_profile_active() -> bool:
    ...
    """
    from csuite.onboarding.profile_builder import load_or_create_profile
...
async def run_scheduler(
    ...
    *,
    ...
    try:
        from csuite.clients.rotation import (
            clear_stale_rotation_marker,
            seed_client_rotation,
        )
        from csuite.config import get_settings as _gs
...
async def _execute_action(
    ...
    if action.department:
        from csuite.departments.authority import gate_action, propose_via_alert
        from csuite.people.models import AuthorityScope
        from csuite.people.registry import get_person
        from csuite.scheduler.action_phrasing import describe_executive_action
    ...
    if action.kind == "dept_cadence":
        ...
        try:
            ...
            from csuite.config import get_settings as _get_settings
            from csuite.departments.cadence import enqueue_next
            from csuite.knowledge.store import ChromaDBStore as _ChromaDBStore
            from csuite.workflows.department_check_in import (
                DepartmentCheckInInput,
                DepartmentCheckInWorkflow,
            )
            from csuite.workflows.persistence import (
                complete_run,
                create_run,
                fail_run,
            )
            ...
            if run_id:
                try:
                    from csuite.workflows.persistence import fail_run
    ...
    if action.kind == "nudge_scan":
        from csuite.scheduler.nudge_engine import (
            enqueue_next_scan,
            run_nudge_scan,
        )
        try:
    ...
    if action.kind == "proactive_nudge" and action.assigned_to_person_id is not None:
        from csuite.audit import log_event as audit_log
        from csuite.people.channel import (
            next_available_window,
            prefer_channel_for,
        )
        from csuite.scheduler.nudge_engine import _leave_end_for
    ...
    try:
        from csuite.knowledge.retriever import retrieve
        from csuite.memory.episodic import format_for_prompt
        from csuite.onboarding.profile_builder import load_or_create_profile
        from csuite.orchestrator.executive import Executive
        from csuite.orchestrator.session import Session
...
def _rotation_pause_active() -> bool:
    ...
    """
    try:
        from csuite.clients.rotation import rotation_in_progress
...
def _has_pending_brief(kind: str) -> bool:
    ...
    """
    from csuite.memory.episodic import _get_conn, _resolve_db_path
...
def seed_principal_briefs() -> int:
    ...
    from csuite.memory.episodic import insert_scheduled_action
...
async def _deliver_to_principal(text: str) -> tuple[bool, str]:
    ...
    """
    from csuite.people.store import find_principal_person
    ...
    for channel in ordered_channels:
        ...
        try:
            if channel == "slack_dm" and principal.slack_user_id:
                from csuite.orchestrator.schedule_tools import handle_send_slack_dm
                ...
                from csuite.orchestrator.schedule_tools import handle_send_discord_dm
                ...
                from csuite.orchestrator.schedule_tools import (
...
async def _run_onboarding_ramp(action: ScheduledAction, now: datetime) -> None:
    ...
    """
    from csuite.orchestrator.schedule_tools import handle_message_person
    from csuite.staff_onboarding import store as ob_store
    from csuite.staff_onboarding.ramp_scheduler import enqueue_ramp
...
async def _send_team_notice(department: str, text: str) -> bool:
    ...
    if not department:
        return False
    from csuite.orchestrator.broadcast_tools import post_department_notice
...
async def _run_onboarding_checkin(action: ScheduledAction, now: datetime) -> None:
    ...
    from csuite.orchestrator.schedule_tools import handle_message_person
    from csuite.staff_onboarding import store as ob_store
    from csuite.staff_onboarding.orchestration import phase_for_milestone
    from csuite.staff_onboarding.service import _PHASE_ORDER
...
async def _run_dynamic_workflow(action: ScheduledAction, now: datetime) -> None:
    ...
    from csuite.workflows import get_workflow
    from csuite.workflows.dynamic_cadence import (
        schedule_dynamic_workflow_cadence,
    )
    from csuite.workflows.dynamic_store import get_definition
    from csuite.workflows.persistence import complete_run, create_run, fail_run
...
async def _run_principal_brief(action: ScheduledAction, now: datetime) -> None:
    ...
    from csuite.config import get_settings
    from csuite.knowledge.store import ChromaDBStore
    from csuite.workflows import WORKFLOW_REGISTRY
    from csuite.workflows.persistence import (
        complete_run,
        create_run,
        fail_run,
