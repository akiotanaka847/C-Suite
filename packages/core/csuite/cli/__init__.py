...
@click.group()
def cli() -> None:
    """C-Suite — your AI-powered virtual executive team."""
    pass
...
async def _ask(question: str) -> None:
    ...
    if profile.is_empty():
        console.print(
            "[yellow]No company profile found. Run 'csuite onboard' to set one up.[/yellow]"
        )
...
async def _chat() -> None:
    ...
    if profile.is_empty():
        console.print(
            "[yellow]No company profile found. Run 'csuite onboard' first.[/yellow]\n"
        )
    else:
    ...
    console.print("[bold]C-Suite[/bold] — type 'exit' to quit\n")
...
async def _ingest_oer(
    ...
    from csuite.knowledge.external_sources import ingest_all
...
async def _consolidate_initiatives(apply_changes: bool) -> None:
    from csuite.memory.initiatives_consolidation import (
        apply_clusters,
        propose_clusters,
    )
...
async def _onboard() -> None:
    from csuite.onboarding.profile_builder import build_and_save_profile
    from csuite.onboarding.wizard import (
        TOTAL_STEPS,
        WizardState,
    ...
    console.print("[bold]C-Suite Onboarding[/bold]\n")
    ...
    console.print(
        ...
        "You can now start chatting: [bold]csuite chat[/bold]"
    )
