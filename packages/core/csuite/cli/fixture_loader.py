...
# so the in-package ``csuite/fixtures/`` module (generated-fixture store
...
def reconcile_honcho_workspace_on_startup(settings: Any) -> None:
    ...
    """
    try:
        from csuite.clients.slots import get_active_client
        from csuite.memory.honcho_client import (
...
async def _load_from_dir(
    ...
    async with _FIXTURE_OP_LOCK:
        ...
        try:
            from csuite.clients.slots import park_active_client
        ...
        try:
            ...
            from csuite.clients.slots import CLIENT_WORKSPACE_PREFIX
            from csuite.memory.honcho_client import (
            ...
            demo_ws = f"csuite-fixture-{fixture_name}-{uuid.uuid4().hex[:8]}"
...
async def load_fixture_any(fixture_name: str, settings: Any) -> dict[str, Any]:
    ...
    from csuite.fixtures import generator as fixtures_generator
    from csuite.fixtures import store as fixtures_store
...
async def _apply_state_from_source(source_dir: Path, settings: Any) -> dict[str, Any]:
    ...
    from csuite.memory.episodic import DB_PATH as EPISODIC_DB_PATH
    from csuite.monitoring import store as monitoring_store
...
async def reset_all_state(
    ...
    async with _FIXTURE_OP_LOCK:
        ...
        from csuite.knowledge.skills_index import SKILLS_COLLECTION
        ...
        from csuite.people import store as people_store
        ...
        try:
            from csuite.clients.slots import _active_client_sentinel
...
def _seed_episodic_memory(memory_path: Path, settings: Any) -> dict[str, int]:
    ...
    from csuite.memory.episodic import DB_PATH as _DEFAULT_DB_PATH
...
def _seed_departments(dept_path: Path) -> int:
    ...
    from csuite.departments import store as dept_store
    from csuite.departments.models import AuthorityLevel
