...
from csuite import mcp_server
from csuite.api.routes import (
    agents,
    alerts,
    architecture,
    artifacts,
...
from csuite.integrations.google_chat import router as google_chat_router
from csuite.integrations.telegram_bot import router as telegram_router


class _OELogFormatter(logging.Formatter):
    """Compact, scannable formatter for csuite logs.

    - Strips the redundant `csuite.` prefix from logger names.
...
def _configure_logging() -> None:
    """Send `csuite.*` logs to stdout at INFO level.

    Attaches the handler directly to the `csuite` logger (not root)
    with propagate=False, so uvicorn's `dictConfig` clobbering the root
    handler list doesn't silence us.
    """
    level = os.environ.get("LOG_LEVEL", "INFO").upper()
    app_logger = logging.getLogger("csuite")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    from csuite.alerts.store import initialize_db as initialize_alerts_db
    from csuite.audit import AuditLogger, set_audit_logger
    from csuite.config import get_settings
    from csuite.knowledge.loader import seed_builtin_knowledge, seed_failures
    from csuite.knowledge.skills_index import seed_builtin_skills
    from csuite.knowledge.store import ChromaDBStore
    from csuite.memory.episodic import initialize_db

    # Re-assert logging config AFTER uvicorn's own dictConfig, then announce.
    _configure_logging()
    logging.getLogger("csuite").info(
        "csuite API starting (LOG_LEVEL=%s) — chat turn logs will appear below",
    ...
    from csuite.fixtures.store import initialize_db as initialize_fixtures_db
    initialize_fixtures_db()

    from csuite.agents.overrides import initialize_overrides_db
    ...
    from csuite.people.store import initialize_db as initialize_people_db
    ...
    from csuite.talent.store import initialize_db as initialize_talent_db
    ...
    from csuite.staff_onboarding.store import (
    ...
    from csuite.staff_onboarding.seed import seed_default_templates
    ...
    from csuite.departments.store import (
        initialize_db as initialize_departments_db,
    )
    from csuite.departments.store import seed_default_departments
    initialize_departments_db()
    seed_default_departments()

    from csuite.departments.completeness import check_org_completeness
    _org_warnings = check_org_completeness()
    for _w in _org_warnings:
        logging.getLogger("csuite").warning("org-completeness: %s", _w)

    from csuite.departments.cadence import (
        bootstrap_cadences,
    ...
    if settings.nudge_scan_enabled:
        from csuite.scheduler.nudge_engine import bootstrap_nudge_scan
    ...
    if settings.watchlist_research_enabled:
        from csuite.monitoring.research.scheduler import (
    ...
    from csuite.cli.fixture_loader import reconcile_honcho_workspace_on_startup
    reconcile_honcho_workspace_on_startup(settings)

    from csuite.knowledge.external_sources import load_manifest
    from csuite.knowledge.review_store import ReviewStore
    ...
    from csuite.evals.persistence import (
        initialize_eval_runs_db,
        initialize_user_scenarios_db,
    )
    from csuite.workflows.dynamic_store import initialize_dynamic_workflows_db
    from csuite.workflows.persistence import initialize_runs_db
    ...
    if settings.mcp_enabled:
        from csuite.orchestrator.mcp_gateway import MCPGateway, set_active_gateway
        ...
        from csuite.integrations.email_poller import run_email_poller
        email_poller_task = asyncio.create_task(run_email_poller(gateway))
    else:
        app.state.mcp_gateway = None

    if settings.scheduler_enabled:
        from csuite.scheduler import run_scheduler
    ...
    from csuite.workflows.resumer import run_resumer
    ...
    if settings.discord_bot_token:
        _discord_log = logging.getLogger("csuite")
        try:
            from csuite.integrations.discord_bot import create_discord_bot
        except ImportError:
    ...
    if discord_bot is not None or discord_bot_task is not None:
        ...
        try:
            ...
            logging.getLogger("csuite").warning(
...
def create_app() -> FastAPI:
    ...
    # "https://csuite-ui-dev.fly.dev,https://exec.mycompany.com").
    extra_origins = [
        o.strip()
    ...
    # Expose C-Suite as an MCP server at /mcp (Streamable-HTTP). Gated
