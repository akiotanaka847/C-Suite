...
def _clear_decision_alert(instance_id: int, status: str) -> None:
    ...
    """
    from csuite.alerts.store import set_status_by_external
    from csuite.memory.decision_ledger import (
...
async def _execute_booking(
    ...
    from csuite.orchestrator.calendar_tools import _do_create_event
...
async def approve_decision(instance_id: int, body: ApproveBody) -> DecisionInstance:
    ...
    if not recorded:
        ...
        if external_event_id:
            try:
                from csuite.orchestrator.calendar_tools import _do_delete_event
