...
``csuite.clients.slots``, which owns validation, the shared
...
def _raise_for(exc: Exception) -> None:
    from csuite.clients.slots import (
...
async def list_clients() -> dict:
    from csuite.cli.fixture_loader import get_fixture_status
    from csuite.clients.rotation import rotation_in_progress
    from csuite.clients.slots import get_active_client, list_client_slots
...
async def create_client(req: CreateClientRequest) -> dict:
    from csuite.clients.slots import ClientSlotError, create_client_slot
    from csuite.config import get_settings
...
def _extract_intake_upload(filename: str, content: bytes) -> tuple[str, str]:
    ...
    """
    from csuite.knowledge.loader import extract_text_from_file
...
async def generate_client(
    ...
    """
    from csuite.clients.slots import derive_client_slug
    from csuite.config import get_settings
...
async def save_client() -> dict:
    ...
    from csuite.clients.slots import ClientSlotError, save_active_client
    from csuite.config import get_settings
...
async def activate_client(slug: str, request: Request) -> dict:
    ...
    from csuite.clients.slots import ClientSlotError, activate_client_slot
    from csuite.config import get_settings
...
async def clients_cockpit() -> dict:
    ...
    from csuite.clients.cockpit import practice_overview
    from csuite.config import get_settings
...
async def patch_client_meta(slug: str, req: ClientMetaPatch) -> dict:
    ...
    from csuite.clients.slots import ClientSlotError, update_client_meta
    from csuite.config import get_settings
...
async def delete_client(slug: str) -> dict:
    from csuite.clients.slots import ClientSlotError, delete_client_slot
    from csuite.config import get_settings
