...
from csuite.providers import allowed_models_for as _allowed_models_for  # noqa: E402
...
def _role_default(name: str) -> str:
    ...
    from csuite.orchestrator.router import SPECIALIST_DESCRIPTIONS
...
def _agent_registry() -> dict[str, Any]:
    ...
    if _executive_proxy is None:
        from csuite.agents.executive_proxy import ExecutiveProxy
        _executive_proxy = ExecutiveProxy()
    if _quality_judge_agent is None:
        from csuite.agents.quality_judge import QualityJudgeAgent
        _quality_judge_agent = QualityJudgeAgent()
    if _utility_fast_agent is None:
        from csuite.agents.utility_fast import UtilityFastAgent
        _utility_fast_agent = UtilityFastAgent()
    if _research_council_agent is None:
        from csuite.agents.research_council import ResearchCouncilAgent
        _research_council_agent = ResearchCouncilAgent()
    if _fixture_generator_agent is None:
        from csuite.agents.fixture_generator import FixtureGeneratorAgent
        _fixture_generator_agent = FixtureGeneratorAgent()
    if _engagement_intake_agent is None:
        from csuite.agents.engagement_intake import EngagementIntakeAgent
...
class _ExecutiveDefaults:
    ...
    @staticmethod
    def prompt() -> str:
        from csuite.prompts.executive_persona import EXECUTIVE_PERSONA_PROMPT
...
def _domains_for(name: str) -> list[str]:
    from csuite.knowledge.retriever import DOMAIN_ALIASES
...
def _build_detail(name: str) -> AgentDetail:
    from csuite.monitoring.research.prompts import default_research_focus
...
def patch_agent(agent_id: str, patch: AgentPatch) -> AgentDetail:
    ...
    if "voice_persona_slug" in raw and patch.voice_persona_slug is not None:
        from csuite.personas.loader import persona_exists
