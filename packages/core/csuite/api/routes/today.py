...
from csuite.briefing.onboarding_digest import (
    OnboardingBriefItem,
    build_onboarding_brief_items,
)
from csuite.briefing.talent_digest import TalentBriefItem, build_talent_brief_items
from csuite.clients.cockpit import ClientCockpitCard, format_practice_for_today

if TYPE_CHECKING:
    from csuite.people.models import Person
...
class ProposalItem(BaseModel):
    ...
    created_at: str
    topic_tags: list[str]
    # Presentation signals (see csuite.briefing.ranking). `score` is
...
def _parse_decision_instance_id(topic_tags: list[str]) -> int | None:
    ...
    """
    from csuite.memory.decision_ledger import DECISION_INSTANCE_TAG_PREFIX
...
def _build_today(
    ...
    """
    from csuite.alerts.store import list_alerts
    from csuite.departments.store import list_departments
    from csuite.memory.episodic import (
        last_contact_at_by_person,
        list_awaiting_replies_by_person,
    )
    from csuite.people import insights, insights_cache
    from csuite.people.channel import is_reachable_now, next_window_for
    from csuite.people.store import list_people
    from csuite.workflows.persistence import list_awaiting_runs
    ...
    from csuite.briefing.ranking import score_and_categorize
    ...
    from csuite.memory.episodic import list_pending_scheduled_actions
...
def _classify_scheduled_action(kind: str, channel: str) -> str:
    ...
    csuite.memory.episodic.ScheduledAction. Unknown combinations
...
def _build_channel_lookup() -> dict[tuple[str, str], str]:
    ...
    """
    from csuite.people import registry as people_registry
...
def _build_activity(limit: int) -> ActivityResponse:
    ...
    """
    from csuite.alerts import store as alerts_store
    from csuite.memory import decision_ledger
    from csuite.memory.episodic import (
        get_recent_advice,
    ...
    from csuite.workflows import persistence as wf_persistence
    ...
    from csuite.departments import store as dept_store
    propose_only_depts = {
        d.config.slug
...
def _build_daily_activity(days: int) -> DailyActivityResponse:
    ...
    """
    from csuite.memory.episodic import count_activity_by_day
...
def _narrative_inputs(
    ...
    """
    from csuite.briefing import narrative_cache
...
async def _regen_briefing_narrative(
    ...
    """
    from csuite.briefing import narrative_cache
    from csuite.briefing.narrative import synthesize_briefing_narrative
...
async def get_today(request: Request, background_tasks: BackgroundTasks) -> TodayResponse:
    ...
    from csuite.api.routes.chat import _resolve_caller_person_id
