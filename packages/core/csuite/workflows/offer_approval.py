...
talent specialist), persists it as an :class:`~csuite.talent.models.Offer`,
...
``csuite.talent.offers``. The ``result`` and ``artifact`` events are
...
from csuite.orchestrator.router import route_to_specialist
from csuite.talent import store as talent_store
from csuite.talent.models import (
    Candidate,
    CandidateStage,
    Engagement,
    Offer,
...
from csuite.talent.offers import gate_context_summary
from csuite.talent.reminders import (
    ReminderContext,
    artifact_header,
    person_channel_raw,
...
from csuite.workflows.wait_for_human import WaitForHumanEvent
...
def _resolve_approver(ctx: ReminderContext) -> Any:
    ...
    from csuite.people.store import find_approvers
...
def _department_slug(engagement: Engagement) -> str:
    ...
    """
    from csuite.departments.registry import get_state
