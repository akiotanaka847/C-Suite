...
from csuite.cli.fixture_loader import (
    _FIXTURE_OP_LOCK,
    _SAFE_NAME_RE,
    get_fixture_status,
    snapshot_user_state,
...
CLIENT_WORKSPACE_PREFIX = "csuite-client-"
...
def _episodic_db_path() -> Path:
    ...
    from csuite.memory.episodic import DB_PATH
...
def list_client_slots(settings: Any) -> list[dict[str, Any]]:
    ...
    for slot in sorted(root.iterdir()):
        ...
        if profile_path.exists():
            try:
                from csuite.memory.company_profile import CompanyProfile
...
def derive_client_slug(display_name: str, settings: Any) -> str:
    ...
    from csuite.fixtures.generator import derive_slug
...
def _ensure_schemas() -> None:
    ...
    from csuite.alerts.store import initialize_db as init_alerts
    from csuite.departments.store import initialize_db as init_departments
    from csuite.fixtures.store import initialize_db as init_fixtures
    from csuite.knowledge.review_store import ReviewStore
    from csuite.memory.episodic import initialize_db as init_episodic
    from csuite.monitoring.store import initialize_db as init_monitoring
    from csuite.people.store import initialize_db as init_people
    from csuite.staff_onboarding.store import initialize_db as init_onboarding
    from csuite.talent.store import initialize_db as init_talent
...
def _reseed_blank_defaults(*, seed_departments: bool = True) -> None:
    ...
    try:
        from csuite.scheduler.runner import seed_principal_briefs

        seed_principal_briefs()
    except Exception:
        logger.exception("client-slots: seed_principal_briefs failed")
    try:
        from csuite.departments.cadence import bootstrap_cadences
...
async def _rebuild_vector_state(settings: Any, app_state: Any | None) -> int:
    ...
    """
    from csuite.knowledge.loader import ingest_file
    from csuite.knowledge.skills_index import SKILLS_COLLECTION, index_skill
    from csuite.knowledge.skills_repo import list_skills
...
def _set_honcho_client_workspace(slug: str) -> str | None:
    ...
    """
    try:
        from csuite.memory.honcho_client import set_active_workspace_id
...
def _write_generated_slot(
    ...
    """
    from csuite.fixtures.generator import (
...
async def activate_client_slot(
    ...
    async with _FIXTURE_OP_LOCK:
        ...
        try:
            from csuite.clients.rotation import seed_client_rotation
