...
from csuite.audit import bind_turn, clear_turn, set_turn
from csuite.audit import log_event as audit_log
from csuite.audit.redaction import (
    audit_tool_input,
...
from csuite.memory.honcho_client import ReasoningLevel as HonchoReasoningLevel
from csuite.orchestrator.action_chips import summarize_action
from csuite.orchestrator.alert_tools import (
    CREATE_ALERT_TOOL,
    handle_create_alert,
)
from csuite.orchestrator.artifact_tools import (
    DRAFT_ARTIFACT_TOOL,
    handle_draft_artifact,
)
from csuite.orchestrator.broadcast_tools import (
    BROADCAST_TOOL_HANDLERS,
    BROADCAST_TOOLS,
)
from csuite.orchestrator.calendar_tools import (
    CALENDAR_TOOL_HANDLERS,
    CALENDAR_TOOLS,
)
from csuite.orchestrator.debug_events import DebugCollector
from csuite.orchestrator.department_tools import (
    DEPARTMENT_TOOL_HANDLERS,
    DEPARTMENT_TOOLS,
)
from csuite.orchestrator.form_tools import (
    FORM_TOOL_HANDLERS,
    FORM_TOOLS,
    PROPOSE_FORM_VALUES,
    build_form_patch_event,
)
from csuite.orchestrator.mcp_gateway import MCP_TOOL_NAMES, MCP_TOOLS, MCPGateway
from csuite.orchestrator.onboarding_tools import (
    ONBOARDING_TOOL_HANDLERS,
    ONBOARDING_TOOLS,
)
from csuite.orchestrator.people_tools import (
    PEOPLE_TOOL_HANDLERS,
    PEOPLE_TOOLS,
)
from csuite.orchestrator.research_tools import (
    RESEARCH_TOOL_HANDLERS,
    RESEARCH_TOOLS,
)
from csuite.orchestrator.router import (
    SPECIALIST_TOOLS,
...
from csuite.orchestrator.skills_tools import SKILL_TOOL_HANDLERS, SKILL_TOOLS
from csuite.orchestrator.talent_tools import (
    TALENT_TOOL_HANDLERS,
    TALENT_TOOLS,
)
from csuite.orchestrator.watchlist_tools import (
    WATCHLIST_TOOL_HANDLERS,
    WATCHLIST_TOOLS,
)
from csuite.orchestrator.web_search_tool import (
    WEB_SEARCH_TOOL_NAME,
    build_web_search_tool,
)
from csuite.orchestrator.workflow_authoring_tools import (
    WORKFLOW_AUTHORING_TOOL_HANDLERS,
    WORKFLOW_AUTHORING_TOOLS,
)
from csuite.orchestrator.workflow_run_tools import (
    WORKFLOW_RUN_TOOL_HANDLERS,
    WORKFLOW_RUN_TOOLS,
)
from csuite.prompts.cache_manager import build_system_blocks
...
def _build_current_speaker_block(person_id: int | None) -> str | None:
    ...
    if person_id is None:
        return None
    try:
        from csuite.people.store import get_person
...
def _sync_consulted_departments_to_honcho(
    ...
    *,
    ...
    if not consulted_specialists:
        return
    from csuite.departments.registry import slug_for_specialist
    from csuite.memory.honcho_client import sync_department_turn
...
class Executive:
    ...
    async def stream_chat(
        ...
        :class:`csuite.people.models.Person` and pass the id here;
        ...
        try:
            from csuite.agents.overrides import (
                EXECUTIVE_AGENT_ID,
            )
            from csuite.agents.overrides import (
                get_override as _get_override,
            )
            from csuite.personas.loader import get_active_body as _get_voice_body
        ...
        with set_turn(session_id=session.session_id, turn_id=turn_id):
            ...
            if peer_memory_context is None:
                from csuite.memory.honcho_client import prefetch as _honcho_prefetch
        ...
        with set_turn(session_id=session.session_id, turn_id=turn_id):
            from csuite.memory.honcho_client import sync_turn as _honcho_sync
            _honcho_sync(
                user_message,
                full_response,
                person_id=person_id,
    ...
    async def stream_chat_with_committee(
        ...
        from csuite.orchestrator.committee import Committee
        committee = Committee(
        ...
        from csuite.prompts.committee_prompts import build_revision_user_turn
