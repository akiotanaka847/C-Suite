...
def _guard_outbound(*, tool: str, channel: str, channel_ref: str, text: str) -> str | None:
    ...
    toward the rate cap.
    """
    from csuite.orchestrator.outbound_guard import check_outbound_allowed
...
def _resolve_recipient_person_id(channel: str, channel_ref: str) -> int | None:
    ...
    try:
        from csuite.people.store import find_person_by_channel_ref
...
def _record_outbound_context(
    *,
    ...
    try:
        ...
        from csuite.memory.episodic import insert_outbound_context
...
async def handle_schedule_followup(tool_input: dict[str, Any]) -> str:
    ...
    try:
        ...
        from csuite.audit import log_event as audit_log_err
        audit_log_err(
            "scheduled_action",
...
async def handle_send_telegram_message(tool_input: dict[str, Any]) -> str:
    from csuite.config import get_settings
    from csuite.integrations.telegram_bot import send_message
...
async def handle_send_discord_dm(tool_input: dict[str, Any]) -> str:
    from csuite.config import get_settings
    from csuite.integrations.discord_bot import send_dm
...
async def _alert_undeliverable_person(
    ...
    try:
        from csuite.orchestrator.alert_tools import handle_create_alert
