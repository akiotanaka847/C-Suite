...
_INSTRUCTIONS = (
    "C-Suite exposed as an MCP server. It surfaces a company's "
...
def _enrich_matches(matches: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    """
    from csuite.talent import store as talent_store
...
async def today_activity() -> str:
    from csuite.api.routes.today import _build_activity
...
async def memory_decisions() -> str:
    from csuite.memory.episodic import list_decisions
...
async def memory_initiatives() -> str:
    from csuite.memory.episodic import list_initiatives
...
async def memory_advice() -> str:
    from csuite.memory.episodic import list_advice
...
async def talent_engagements() -> str:
    from csuite.briefing.talent_digest import build_talent_brief_items
...
async def consult_specialist(
    ...
    """Consult one of C-Suite's specialist executives for domain analysis.
...
async def search_knowledge(query: str) -> str:
    """Search C-Suite's curated MBA knowledge base and indexed company docs.
...
async def list_workflows() -> str:
    ...
    tool-call's timeout.
    """
    from csuite.workflows import list_workflows as _list
...
async def list_candidates(engagement_id: int, stage: str = "") -> str:
    ...
    """
    from csuite.talent import store as talent_store
    from csuite.talent.models import CandidateStage
...
async def match_candidates(engagement_id: int, limit: int = 10) -> str:
    ...
    """
    from csuite.talent import graph as talent_graph
    from csuite.talent import store as talent_store
