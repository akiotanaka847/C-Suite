...
from csuite.integrations.response_gate import (
    GateDecision,
    _parse_gate_output,
    should_respond,
)
...
async def test_yes_open_floor_question_in_multi_party_thread():
    ...
    with (
        patch("csuite.providers.get_provider", return_value=provider),
        patch("csuite.config.get_settings", return_value=_mocked_settings()),
    ):
