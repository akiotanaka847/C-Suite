...
import csuite.integrations.google_chat as gc_module
from csuite.integrations.google_chat import _process_and_reply, send_reply  # noqa: F401
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    with patch("csuite.integrations.google_chat.get_settings", return_value=_make_settings()):
...
def test_unconfigured_returns_503() -> None:
    ...
    app.include_router(gc_module.router)
    with patch(
        "csuite.integrations.google_chat.get_settings",
...
def test_process_calls_alert_pipeline_with_correct_fields(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "csuite.alerts.pipeline.schedule_evaluation",
        lambda event: recorded.append(event),
    )
    monkeypatch.setattr("csuite.knowledge.retriever.retrieve", lambda **_: "")
    monkeypatch.setattr("csuite.memory.episodic.format_for_prompt", lambda: "")

    profile = SimpleNamespace(is_empty=lambda: True)
    monkeypatch.setattr(
        "csuite.onboarding.profile_builder.load_or_create_profile", lambda: profile
    )
    ...
    with (
        patch("csuite.orchestrator.executive.Executive", return_value=mock_exec),
        patch("csuite.integrations.google_chat.send_reply", new=fake_send_reply),
    ):
        asyncio.run(
            _process_and_reply(
...
def test_process_sends_error_reply_on_executive_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("csuite.alerts.pipeline.schedule_evaluation", lambda _: None)
    monkeypatch.setattr("csuite.knowledge.retriever.retrieve", lambda **_: "")
    monkeypatch.setattr("csuite.memory.episodic.format_for_prompt", lambda: "")
