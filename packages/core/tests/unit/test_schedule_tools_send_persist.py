...
from csuite.memory.episodic import initialize_db, list_scheduled_actions
...
def settings_with_tokens(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "csuite.config.get_settings", lambda: _S()
    )
...
def test_send_telegram_message_persists_done_row(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "csuite.people.store.find_person_by_telegram_chat_id",
        lambda _ref: object(),
    )
    with patch(
        "csuite.integrations.telegram_bot.send_message",
        new=AsyncMock(return_value=None),
    ):
        result = asyncio.run(
...
def test_persist_failure_does_not_break_send(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "csuite.memory.episodic.insert_scheduled_action",
...
def test_correlated_persist_and_audit_failure_does_not_break_send(
    ...
    import csuite.audit as audit_mod
    ...
    monkeypatch.setattr("csuite.audit.log_event", _conditional_log)
    with patch(
        "csuite.integrations.discord_bot.send_dm",
        new=AsyncMock(return_value=None),
    ):
