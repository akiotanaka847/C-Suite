...
from csuite.monitoring.research import scheduler as research_scheduler
...
def db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("csuite.people.store.DB_PATH", db_path)
...
def test_state_hash_changes_when_initiative_added(db: Path) -> None:
    from csuite.memory.episodic import store_initiative
...
def _make_workflow_stub(
    ...
    async def fake_run(*, inputs: Any, store: Any):
        from csuite.workflows.base import WorkflowEvent
        yield WorkflowEvent(
            type="result",
            data={
                "findings": findings,
...
async def test_scan_skips_when_state_unchanged(
    ...
    monkeypatch.setitem(
        __import__(
            "csuite.workflows", fromlist=["WORKFLOW_REGISTRY"]
        ).WORKFLOW_REGISTRY,
...
async def test_scan_skips_after_run_that_mutates_watchlist(
    ...
    from csuite.audit import get_audit_logger
...
async def test_staleness_floor_disabled_keeps_skipping(
    ...
    monkeypatch.setattr("csuite.config.get_settings", lambda: _Stub())
...
def test_clear_research_run_history_only_touches_research_rows(
    ...
    from csuite.audit import log_event
