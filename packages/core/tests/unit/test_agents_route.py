...
from csuite.agents import overrides as ov_mod  # noqa: E402
from csuite.api.routes import agents as agents_route  # noqa: E402
...
def test_test_endpoint_passes_draft_to_analyze(client: TestClient) -> None:
    ...
    with patch("csuite.agents.base.get_provider", return_value=fake_provider):
        res = client.post(
            "/agents/cso/test",
            json={
                "query": "Hi",
...
def test_executive_test_endpoint_uses_draft_prompt(client: TestClient) -> None:
    ...
    with patch(
        "csuite.providers.get_provider", return_value=fake_provider
    ):
        res = client.post(
