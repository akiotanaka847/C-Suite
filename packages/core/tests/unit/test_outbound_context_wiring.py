...
from csuite.integrations.discord_bot import _handle_message
from csuite.memory.episodic import find_open_outbound_context, initialize_db
from csuite.orchestrator import schedule_tools
from csuite.orchestrator.schedule_tools import current_session
...
def isolated_db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("csuite.memory.episodic.DB_PATH", db_path)
    return db_path
...
def settings_with_tokens(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("csuite.config.get_settings", lambda: _S())
...
async def test_discord_dm_inbound_hydrates_from_linkage() -> None:
    ...
    with (
        ...
        patch(
            "csuite.integrations.inbound_hydration.hydrate_user_message",
