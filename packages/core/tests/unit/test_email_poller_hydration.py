...
import csuite.integrations.email_poller as poller
import csuite.integrations.inbound_hydration as inbound_hydration
from csuite.memory import session_store
from csuite.memory.episodic import initialize_db, insert_outbound_context
from csuite.memory.session_store import create_session, save_message
...
def _capture_user_message(from_addr: str, raw_email_body: str) -> str:
    ...
    from csuite.memory import company_profile as _cp

    with (
        patch("csuite.orchestrator.executive.Executive", _StubExecutive),
        patch(
