...
def _isolated(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    from csuite.alerts import store as alert_store
    from csuite.memory import episodic
    db = tmp_path / "test.db"
...
def _call(tool_input: dict[str, Any], settings: Any | None = None, gateway: Any = None) -> dict[str, Any]:
    ...
    with (
        patch("csuite.config.get_settings", return_value=s),
        patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=gw),
        patch("csuite.departments.authority.gate_action") as mock_gate,
    ):
...
def test_auto_execute_creates_event_and_marks_executed() -> None:
    ...
    with (
        ...
        patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=gw),
        patch("csuite.departments.authority.gate_action") as mock_gate,
        patch("csuite.memory.decision_ledger.get_class_mode", return_value="auto_execute"),
    ):
...
def test_auto_execute_marks_failed_on_mcp_error() -> None:
    ...
    from csuite.memory.decision_ledger import list_instances
...
def test_propose_creates_linked_briefing_alert(_isolated: Path) -> None:
    from csuite.alerts.store import get_alert_by_external
...
def test_cancel_event_marks_reversed() -> None:
    from csuite.memory.decision_ledger import mark_resolved
...
def test_cancel_nonexistent_returns_error() -> None:
    with patch("csuite.orchestrator.mcp_gateway.get_active_gateway",
...
def test_instant_meeting_tool_gated_on_calendar_configured() -> None:
    from csuite.orchestrator.calendar_tools import CALENDAR_TOOLS
    from csuite.orchestrator.schedule_tools import (
        filter_tools_for_configured_channels,
    )

    with patch(
        "csuite.orchestrator.mcp_gateway.get_active_gateway",
        return_value=_fake_gateway(),
    ):
