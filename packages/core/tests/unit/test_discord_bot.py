...
from csuite.integrations.discord_bot import (
    CLASSIFY_DM,
    CLASSIFY_MENTION,
...
from csuite.integrations.response_gate import GateDecision
...
def mock_executive_chat():
    ...
    with (
        patch("csuite.integrations.discord_bot._handle_message") as mock_hm,
    ):
        yield mock_hm
...
async def test_handle_message_calls_audit_log():
    ...
    with (
        patch("csuite.people.store.find_person_by_discord_id", return_value=MagicMock(id=42)),
        patch("csuite.alerts.pipeline.schedule_evaluation"),
        patch("csuite.knowledge.retriever.retrieve", return_value=""),
        patch("csuite.memory.episodic.format_for_prompt", return_value=""),
        patch("csuite.onboarding.profile_builder.load_or_create_profile") as mock_profile,
        patch("csuite.orchestrator.executive.Executive") as MockExec,
        patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=None),
        patch("csuite.orchestrator.session.Session") as MockSession,
        patch("csuite.audit.log_event") as mock_audit,
        patch("csuite.memory.session_store.load_messages", return_value=[]),
        patch("csuite.memory.session_store.create_session"),
        patch("csuite.memory.session_store.save_message"),
        patch("csuite.memory.session_store.update_session_timestamp"),
    ):
...
async def test_handle_message_dm_seeds_channel_ref():
    ...
    with (
        ...
        patch("csuite.orchestrator.executive.Executive") as MockExec,
        patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=None),
        patch("csuite.orchestrator.session.Session", return_value=session_mock),
        patch("csuite.audit.log_event"),
        patch("csuite.memory.session_store.load_messages", return_value=[]),
        patch("csuite.memory.session_store.create_session"),
        patch("csuite.memory.session_store.save_message"),
        patch("csuite.memory.session_store.update_session_timestamp"),
...
async def test_handle_message_resolver_path_sends_ack_and_returns():
    ...
    with (
        patch("csuite.people.store.find_person_by_discord_id", return_value=person_mock),
        patch(
            "csuite.workflows.inbound_resolver.resolve_inbound_message",
            new=AsyncMock(return_value=resolution_mock),
        ),
        patch(
            "csuite.workflows.resumer.apply_resolution",
...
async def test_handle_message_loads_history_and_seeds_conversation():
    ...
    with (
        ...
        patch("csuite.orchestrator.executive.Executive") as MockExec,
        patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=None),
        patch("csuite.orchestrator.session.Session", return_value=session_mock) as MockSession,
        patch("csuite.audit.log_event"),
        patch("csuite.memory.session_store.load_messages", return_value=prior_history) as mock_load,
        patch("csuite.memory.session_store.create_session"),
        patch("csuite.memory.session_store.save_message"),
        patch("csuite.memory.session_store.update_session_timestamp"),
    ):
...
async def test_handle_message_persists_user_and_assistant_messages_after_success():
    ...
    with (
        ...
        patch("csuite.audit.log_event"),
        patch("csuite.memory.session_store.load_messages", return_value=[]),
        patch("csuite.memory.session_store.create_session") as mock_create,
        patch("csuite.memory.session_store.save_message") as mock_save,
        patch("csuite.memory.session_store.update_session_timestamp") as mock_touch,
    ):
        mock_profile.return_value.is_empty.return_value = True
        mock_exec_instance = MagicMock()
...
async def test_generate_thread_title_returns_trimmed_haiku_output():
    ...
    with (
        patch("csuite.providers.get_provider", return_value=fake_provider),
        patch("csuite.config.get_settings") as mock_settings,
    ):
...
def _patches_for_handle(history: list[dict] | None = None):
    ...
    return {
        "person": patch("csuite.people.store.find_person_by_discord_id", return_value=MagicMock(id=42)),
        "alerts": patch("csuite.alerts.pipeline.schedule_evaluation"),
        "retrieve": patch("csuite.knowledge.retriever.retrieve", return_value=""),
        "episodic": patch("csuite.memory.episodic.format_for_prompt", return_value=""),
        "profile": patch("csuite.onboarding.profile_builder.load_or_create_profile"),
        "exec": patch("csuite.orchestrator.executive.Executive"),
        "gateway": patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=None),
        "audit": patch("csuite.audit.log_event"),
        "load": patch("csuite.memory.session_store.load_messages", return_value=history or []),
        "create": patch("csuite.memory.session_store.create_session"),
        "save": patch("csuite.memory.session_store.save_message"),
        "touch": patch("csuite.memory.session_store.update_session_timestamp"),
    }
...
async def test_should_respond_returns_true_on_yes():
    ...
    with (
        patch("csuite.providers.get_provider", return_value=fake_client),
        patch("csuite.config.get_settings") as mock_settings,
    ):
        mock_settings.return_value.anthropic_api_key = "sk-test"
        mock_settings.return_value.routing_model = "claude-haiku-4-5-20251001"
...
async def test_handle_message_gate_skips_when_haiku_says_no():
    ...
    with (
        ...
        patch(
            "csuite.integrations.response_gate.should_respond",
...
async def test_handle_message_gate_bypassed_in_single_human_thread():
    ...
    with (
        ...
        patch("csuite.orchestrator.session.Session", return_value=session_mock),
        p["audit"], p["load"], p["create"], p["save"], p["touch"],
        patch("csuite.integrations.response_gate.should_respond", new=gate),
        patch("csuite.config.get_settings") as mock_settings,
    ):
        mock_settings.return_value.discord_thread_response_gate_enabled = True
        mock_profile.return_value.is_empty.return_value = True
...
async def test_handle_message_router_promoted_double_writes_both_sessions():
    ...
    with (
        ...
        p["audit"], p["load"], p["create"] as mock_create,
        p["save"] as mock_save, p["touch"],
        patch("csuite.integrations.discord_bot._schedule_thread_rename") as mock_rename,
    ):
        mock_profile.return_value.is_empty.return_value = True
        mock_exec_instance = MagicMock()
...
def test_session_title_mention_channel_includes_user():
    ...
    from csuite.integrations.discord_bot import _session_title_for_message
