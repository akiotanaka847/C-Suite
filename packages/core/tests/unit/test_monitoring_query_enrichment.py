...
from csuite.monitoring import enrichment as me
from csuite.monitoring import pipeline as mp
from csuite.monitoring import store as ms
from csuite.monitoring.enrichment import SignalEnrichment, enrich_signal
...
from csuite.monitoring.sources.query import (
    QuerySource,
    _coerce_max_results,
    _QueryResearchAgent,
)
...
def _allow_all_urls(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "csuite.monitoring.sources.query.validate_target_url",
        lambda u: (True, ""),
    )
...
async def test_query_empty_when_web_search_disabled(
    ...
    monkeypatch.setattr(
        "csuite.monitoring.sources.query.build_web_search_tool",
        lambda: None,
    )
    src = QuerySource()
