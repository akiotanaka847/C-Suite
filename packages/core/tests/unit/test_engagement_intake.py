...
from csuite.agents.engagement_intake import (
    ENGAGEMENT_INTAKE_SYSTEM,
    EngagementIntakeAgent,
)
from csuite.fixtures import generator as gen
...
async def test_generate_engagement_bundle_uses_grounded_agent_and_tool(
    ...
    monkeypatch.setattr(
        "csuite.providers.registry.get_provider", lambda model: FakeProvider()
    )
...
def test_engagement_intake_agent_registered_in_council() -> None:
    from csuite.api.routes.agents import _agent_registry
...
def client(tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    from csuite.api.routes import clients as route
