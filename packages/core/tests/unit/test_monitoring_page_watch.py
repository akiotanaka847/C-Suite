...
from csuite.monitoring.models import WatchlistItem
from csuite.monitoring.sources import list_registered_kinds
from csuite.monitoring.sources.page_watch import (
    PageWatchSource,
    _diff,
    _html_to_text,
)
...
def _install(monkeypatch: pytest.MonkeyPatch, page: _Page) -> None:
    ...
    monkeypatch.setattr("csuite.monitoring.sources.page_watch.fetch_bounded", fake_fetch)
    monkeypatch.setattr(
        "csuite.monitoring.sources.page_watch.validate_target_url",
        lambda u: (True, ""),
    )
...
async def test_bad_target_and_fetch_failure_return_empty(
    ...
    monkeypatch.setattr("csuite.monitoring.sources.page_watch.fetch_bounded", boom)
    monkeypatch.setattr(
        "csuite.monitoring.sources.page_watch.validate_target_url",
        lambda u: (True, ""),
    )
...
def _install_xcrawl(monkeypatch: pytest.MonkeyPatch, holder: SimpleNamespace) -> None:
    ...
    monkeypatch.setattr(
        "csuite.monitoring.sources.page_watch.fetch_bounded", boom_fetch,
    )
    monkeypatch.setattr(
        "csuite.integrations.xcrawl_client.scrape", fake_scrape,
    )
