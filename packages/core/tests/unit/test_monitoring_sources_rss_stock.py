...
from csuite.monitoring.sources.rss import RssSource
from csuite.monitoring.sources.stock import (
    StockSource,
    _looks_like_ticker,
    _severity_for_move,
)
...
async def test_rss_emits_signals_for_each_entry(
    ...
    monkeypatch.setattr(
        "csuite.monitoring.sources.rss.fetch_bounded", fake_fetch
    )
    monkeypatch.setattr(
        "csuite.monitoring.sources.rss.validate_target_url",
        lambda u: (True, ""),
    )
...
def test_rss_matches_trigger_keywords() -> None:
    ...
    from csuite.monitoring.models import Signal
    matching = Signal(
        watchlist_id=1,
        source_kind="rss",
...
async def test_rss_rejects_ssrf_target(
    ...
    monkeypatch.setattr(
        "csuite.monitoring.sources.rss.fetch_bounded", should_not_fetch
    )
    src = RssSource()
...
async def test_stock_emits_signal_when_move_exceeds_threshold(
    ...
    monkeypatch.setattr(
        "csuite.monitoring.sources.stock.fetch_bounded", fake_fetch
    )
    monkeypatch.setattr(
        "csuite.monitoring.sources.stock.validate_target_url",
        lambda u: (True, ""),
    )
...
async def test_stock_dedup_distinguishes_up_vs_down(
    ...
    monkeypatch.setattr(
        "csuite.monitoring.sources.stock.fetch_bounded", fetch_up
    )
    s_up = (await src.poll(item))[0]
    monkeypatch.setattr(
        "csuite.monitoring.sources.stock.fetch_bounded", fetch_down
    )
...
async def test_stock_threshold_coercion_handles_bad_input(
    ...
    from csuite.monitoring.sources.stock import _coerce_threshold
