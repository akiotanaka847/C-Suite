...
from csuite.workflows.candidate_screen import (
...
def _run(engagement_id: int, candidate_id: int) -> list:
    ...
    with patch(
        "csuite.workflows.candidate_screen.route_to_specialist",
        new_callable=AsyncMock,
        return_value=_ASSESSMENT,
    ), patch(
        "csuite.workflows.candidate_screen.retrieve", return_value=""
    ):
...
def test_screen_no_score_skips_persistence(db: Path) -> None:
    ...
    ), patch("csuite.workflows.candidate_screen.retrieve", return_value=""):
        asyncio.run(_collect())
