...
from csuite.monitoring.sources.edgar import (
    EdgarSource,
    _allowed_forms,
    _form_matches,
    _severity_for_form,
...
def _install_feed(monkeypatch: pytest.MonkeyPatch, body: bytes) -> dict[str, Any]:
    ...
    monkeypatch.setattr("csuite.monitoring.sources.edgar.fetch_bounded", fake_fetch)
    monkeypatch.setattr(
        "csuite.monitoring.sources.edgar.validate_target_url",
        lambda u: (True, ""),
    )
    return captured
...
async def test_edgar_fetch_failure_returns_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("csuite.monitoring.sources.edgar.fetch_bounded", boom)
    monkeypatch.setattr(
        "csuite.monitoring.sources.edgar.validate_target_url",
        lambda u: (True, ""),
    )
