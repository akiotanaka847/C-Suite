...
from csuite.talent.models import CandidateStage, OfferStatus
from csuite.workflows.offer_approval import (
    OfferApprovalInput,
...
def _run(candidate_id: int, *, dm_mock: AsyncMock | None = None) -> list:
    ...
    with patch(
        "csuite.workflows.offer_approval.route_to_specialist",
        AsyncMock(side_effect=["TERMS_BODY", "CLOSE_PLAN_BODY"]),
    ), patch(
        "csuite.workflows.offer_approval.retrieve", return_value=""
    ), patch(
        "csuite.orchestrator.schedule_tools.handle_message_person", dm,
    ):
