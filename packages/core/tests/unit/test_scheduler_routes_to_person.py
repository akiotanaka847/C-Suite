...
from csuite.people.models import AuthorityScope, AvailabilityWindow
from csuite.scheduler.runner import _execute_action
...
def test_propose_only_creates_alert_no_dispatch() -> None:
    ...
    with patch(
        "csuite.orchestrator.executive.Executive.chat",
...
def test_auto_execute_dispatches() -> None:
    ...
    with patch(
        ...
        "csuite.memory.episodic.format_for_prompt",
        return_value="",
    ):
        asyncio.run(_execute_action(action, gateway=None))
