...
from csuite.api.main import create_app  # noqa: E402
from csuite.workflows import WORKFLOW_REGISTRY, list_workflows  # noqa: E402
from csuite.workflows.persistence import (  # noqa: E402
    complete_run,
    create_run,
    delete_run,
    fail_run,
...
def client(temp_db: Path) -> TestClient:
    ...
    with patch("csuite.workflows.persistence.DB_PATH", temp_db), \
         patch("csuite.api.routes.workflows.list_runs",
               side_effect=lambda **kw: list_runs(db_path=temp_db, **kw)), \
         patch("csuite.api.routes.workflows.get_run",
               side_effect=lambda run_id: get_run(run_id, db_path=temp_db)), \
         patch("csuite.api.routes.workflows.create_run",
               side_effect=lambda **kw: create_run(db_path=temp_db, **kw)), \
         patch("csuite.api.routes.workflows.delete_run",
