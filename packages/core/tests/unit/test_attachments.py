...
def test_build_attachment_output_pdf_extracts_text_and_schedules_ingest():
    ...
    with (
        patch(
            "csuite.integrations.attachments._extract_text",
            return_value=extracted,
        ),
        patch("csuite.integrations.attachments._schedule_ingest") as mock_ingest,
    ):
...
def test_build_attachment_output_truncates_long_text():
    ...
    with (
        ...
        patch("csuite.integrations.attachments._schedule_ingest"),
    ):
...
async def test_process_attachments_skips_failed_download_and_continues():
    ...
    with patch("csuite.integrations.attachments.download_bytes", side_effect=_fake_download):
        extra_text, image_blocks = await process_attachments(items)
...
async def test_process_attachments_concatenates_multiple_texts():
    ...
    with (
        patch("csuite.integrations.attachments.download_bytes", side_effect=_fake_download),
        patch(
            "csuite.integrations.attachments._extract_text",
