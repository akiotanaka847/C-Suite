...
from csuite.workflows.wait_for_human import WaitForHumanResolution
...
def test_resolve_tier1_explicit_message_id(tmp_path: Path) -> None:
    ...
    with patch("csuite.workflows.inbound_resolver.parse_decision", new=AsyncMock(return_value={"decision": "approve", "note": ""})):
        result = _run_resolver(
            channel="slack",
            channel_ref="U123",
            from_person_id=5,
...
def test_resolve_single_candidate_parses_decision(tmp_path: Path) -> None:
    ...
    with patch("csuite.workflows.inbound_resolver.parse_decision", new=AsyncMock(return_value={"decision": "reject", "note": "too expensive"})):
        result = _run_resolver(
            channel="slack",
            channel_ref="U100",
            from_person_id=3,
...
def test_resolve_returns_none_when_multiple_candidates_no_llm(tmp_path: Path) -> None:
    ...
    with patch(
        "csuite.workflows.inbound_resolver._llm_disambiguate",
...
def test_resolve_tier3_high_confidence_returns_match(tmp_path: Path) -> None:
    ...
    ), patch("csuite.workflows.inbound_resolver.parse_decision", new=AsyncMock(return_value={"decision": "approve", "note": ""})):
        result = _run_resolver(
            channel="slack",
            channel_ref="U200",
            from_person_id=2,
...
def test_parse_decision_approve_reject() -> None:
    from csuite.workflows.wait_for_human import parse_decision
    provider = _fake_provider('{"decision": "approve", "note": "looks good"}')
    with patch("csuite.providers.get_provider", return_value=provider):
