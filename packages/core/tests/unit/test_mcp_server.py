"""Unit tests for the C-Suite MCP server (csuite.mcp_server).
...
from csuite.mcp_server import server as mcp_server
...
def test_specialist_enum_matches_registry() -> None:
    ...
    from csuite.orchestrator.router import SPECIALIST_REGISTRY
...
async def test_company_profile_resource_empty() -> None:
    ...
    with patch(
        "csuite.onboarding.profile_builder.load_or_create_profile",
        return_value=fake,
    ):
...
async def test_people_roster_resource_serializes_models() -> None:
    ...
    with patch("csuite.people.store.list_people", return_value=people):
...
async def test_today_briefing_resource_serializes_model() -> None:
    with patch(
        "csuite.api.routes.today._build_today",
...
async def test_today_activity_resource_serializes_model() -> None:
    with patch(
        "csuite.api.routes.today._build_activity",
...
async def test_memory_decisions_resource_serializes_list() -> None:
    with patch(
        "csuite.memory.episodic.list_decisions",
...
async def test_consult_specialist_routes_to_specialist() -> None:
    ...
    with patch("csuite.orchestrator.router.route_to_specialist", new=mock):
...
async def test_search_knowledge_uses_retriever_and_store() -> None:
    ...
    try:
        with patch(
            "csuite.knowledge.retriever.retrieve",
...
async def test_ask_executive_resolves_principal_and_calls_chat() -> None:
    ...
    with (
        ...
        patch(
            "csuite.people.store.find_principal_person",
            return_value=_Person(),
        ),
        patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=None),
        patch("csuite.orchestrator.executive.Executive.chat", new=chat),
    ):
...
async def test_talent_engagements_resource_serializes_rollup() -> None:
    ...
    with patch(
        "csuite.briefing.talent_digest.build_talent_brief_items",
        return_value=items,
    ):
...
async def test_list_candidates_returns_briefs() -> None:
    with (
        patch("csuite.talent.store.get_engagement", return_value=object()),
        patch(
            "csuite.talent.store.list_candidates",
...
async def test_list_candidates_unknown_engagement() -> None:
    with patch("csuite.talent.store.get_engagement", return_value=None):
...
async def test_get_candidate_found_and_missing() -> None:
    with patch(
        "csuite.talent.store.get_candidate", return_value=_Candidate(7)
    ):
        found = await mcp_server.get_candidate(7)
    assert json.loads(found) == {"id": 7, "full_name": "Jane Roe"}

    with patch("csuite.talent.store.get_candidate", return_value=None):
...
async def test_get_candidate_hides_archived() -> None:
    ...
    with patch(
        "csuite.talent.store.get_candidate",
...
async def test_match_candidates_enriches_with_names() -> None:
    ...
    try:
        with (
            patch("csuite.talent.store.get_engagement", return_value=object()),
            patch(
                "csuite.talent.graph.match_candidates_for_engagement",
...
async def test_find_similar_candidates_enriches_with_names() -> None:
    ...
    try:
        ...
        with (
            ...
            patch(
                "csuite.talent.graph.find_similar_candidates",
...
def test_mcp_endpoint_is_gated_by_shared_secret(
    ...
    from csuite.api.main import create_app
