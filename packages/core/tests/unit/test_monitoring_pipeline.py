...
from csuite.alerts.models import AlertSeverity
from csuite.alerts.store import initialize_db as initialize_alerts_db
from csuite.memory.episodic import initialize_db as initialize_episodic_db
from csuite.monitoring import pipeline as mp
from csuite.monitoring import store as ms
from csuite.monitoring.models import (
    MODE_ACTIVE,
    MODE_DRY_RUN,
    OUTCOME_ALERTED,
...
from csuite.monitoring.sources import _REGISTRY as SOURCE_REGISTRY
...
def db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("csuite.alerts.store.DB_PATH", db_path)
...
def test_scan_promotes_signal_to_alert(
    ...
    monkeypatch.setattr(
        "csuite.monitoring.pipeline.schedule_evaluation",
...
def test_schedule_evaluation_failure_records_failed_outcome(
    ...
    monkeypatch.setattr(
        "csuite.monitoring.pipeline.schedule_evaluation", boom
    )
    wl_id = ms.insert_watchlist_item(
...
def test_max_signals_per_scan_cap_enforced(
    ...
    base = get_settings()
    monkeypatch.setattr(
        "csuite.monitoring.pipeline.get_settings",
...
def test_validate_target_url_rejects_ssrf_targets() -> None:
    ...
    from csuite.monitoring.sources._http import validate_target_url as _validate_target_url
    bad = [
        "file:///etc/passwd",
        "javascript:alert(1)",
...
def test_signal_to_alert_event_carries_slug_and_severity_for_triage() -> None:
    ...
    from csuite.alerts.models import AlertSeverity
    from csuite.monitoring.models import Signal, WatchlistItem
    from csuite.monitoring.pipeline import _signal_to_alert_event
...
def test_strip_url_query_drops_tokens() -> None:
    ...
    from csuite.monitoring.pipeline import _safe_url
