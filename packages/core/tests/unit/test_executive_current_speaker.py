...
the principal explicitly. See csuite.orchestrator.executive.
"""
...
def test_build_messages_flags_principal_speaker() -> None:
    ...
    exec_ = Executive()
    with patch("csuite.people.store.get_person", return_value=principal):
...
def test_build_messages_non_principal_speaker_still_guarded() -> None:
    ...
    exec_ = Executive()
    with patch("csuite.people.store.get_person", return_value=person):
        messages = exec_._build_messages(
...
def test_build_messages_no_speaker_block_when_person_unresolved() -> None:
    ...
    with patch("csuite.people.store.get_person", return_value=None):
...
def test_speaker_block_swallows_lookup_errors() -> None:
    ...
    from csuite.orchestrator.executive import _build_current_speaker_block

    with patch("csuite.people.store.get_person", side_effect=RuntimeError("db down")):
