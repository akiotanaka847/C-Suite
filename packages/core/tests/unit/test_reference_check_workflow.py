...
from csuite.workflows.reference_check import (
    ReferenceCheckInput,
...
def _run(candidate_id: int, reference_count: int = 2) -> list:
    ...
    with patch(
        "csuite.workflows.reference_check.route_to_specialist",
        new_callable=AsyncMock,
        side_effect=["RUBRIC_BODY", "OUTREACH_BODY"],
    ), patch("csuite.workflows.reference_check.retrieve", return_value=""):
