...
- ``load_fixture`` switches Honcho's active workspace to a per-fixture
  id (``csuite-fixture-<name>-<uuid8>``) so demo turns sync to a
...
from csuite.cli import fixture_loader
from csuite.knowledge.store import ChromaDBStore
from csuite.memory import honcho_client
...
def settings_stub(tmp_path: Path) -> Any:
    ...
    return type(
        ...
        (),
        {
            ...
            "honcho_workspace_id": "csuite-test",
        },
    )()
...
def _isolate_dbs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("HONCHO_WORKSPACE_ID", "csuite-test")
...
def test_load_fixture_switches_active_workspace(
    ...
    assert active.startswith("csuite-fixture-bombas-")
    assert active != "csuite-test"
...
def test_unload_fixture_deletes_demo_workspace_and_clears_override(
    ...
    honcho_client.set_active_workspace_id(
        "csuite-fixture-bombas-deadbeef", fixture_name="bombas"
    )
    ...
    assert deletes == ["csuite-fixture-bombas-deadbeef"]
    # Override removed → falls back to env default.
    assert honcho_client.get_active_workspace_id() == "csuite-test"
...
def test_reset_deletes_both_active_and_default_when_different(
    ...
    assert deletes == ["csuite-fixture-bombas-deadbeef", "csuite-test"]
    # Override cleared.
    assert honcho_client.get_active_workspace_id() == "csuite-test"
...
def test_reconcile_clears_orphaned_override(
    ...
    """
    honcho_client.set_active_workspace_id("csuite-fixture-bombas-deadbeef")
    assert honcho_client.get_active_workspace_id() == "csuite-fixture-bombas-deadbeef"
