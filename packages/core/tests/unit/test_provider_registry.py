...
from csuite.providers import LLMProvider, get_provider  # noqa: E402
from csuite.providers import registry as registry_mod  # noqa: E402
...
def test_messages_create_proxies_kwargs_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "csuite.providers.anthropic_provider.anthropic.AsyncAnthropic",
...
def test_anthropic_provider_constructs_lazy_only_once(
    ...
    monkeypatch.setattr(
        "csuite.providers.anthropic_provider.anthropic.AsyncAnthropic", _ctor
    )
    registry_mod._reset_for_tests()
    for _ in range(5):
...
def test_claude_model_with_openrouter_off_routes_to_anthropic(
    ...
    monkeypatch.setattr(
        "csuite.providers.registry.get_settings",
        lambda: _settings_stub(enabled=False),
    )
    registry_mod._reset_for_tests()
    from csuite.providers.anthropic_provider import AnthropicProvider
...
def test_claude_model_with_openrouter_on_routes_to_openrouter(
    ...
    registry_mod._reset_for_tests()
    from csuite.providers.openrouter_provider import OpenRouterProvider
...
def test_allowed_models_includes_openrouter_set_only_when_enabled(
    ...
    from csuite.providers import allowed_models
    from csuite.providers.registry import (
...
def test_local_model_routes_to_local_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    from csuite.providers.openai_compatible import OpenAICompatibleProvider
    from csuite.providers.openrouter_provider import OpenRouterProvider
...
def test_allowed_models_includes_local_when_enabled(
    ...
    from csuite.providers import allowed_models
    from csuite.providers.registry import ANTHROPIC_DIRECT_MODELS
