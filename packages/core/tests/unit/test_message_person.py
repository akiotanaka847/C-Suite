...
def test_message_person_routes_to_real_discord_id() -> None:
    ...
    with (
        patch("csuite.config.get_settings", return_value=_settings(discord="tok")),
        patch("csuite.integrations.discord_bot.send_dm", fake_send),
        patch("csuite.audit.log_event"),
        patch("csuite.orchestrator.schedule_tools._record_send_to_activity"),
    ):
...
def test_message_person_honours_preferred_channel() -> None:
    ...
    with (
        patch(
            "csuite.config.get_settings",
            return_value=_settings(discord="tok", telegram="tok"),
        ),
        patch("csuite.integrations.telegram_bot.send_message", fake_tg),
        patch("csuite.integrations.discord_bot.send_dm", fake_discord),
        patch("csuite.audit.log_event"),
        patch("csuite.orchestrator.schedule_tools._record_send_to_activity"),
    ):
...
def test_message_person_no_reachable_channel_falls_back_to_alert() -> None:
    ...
    with (
        patch("csuite.config.get_settings", return_value=_settings(discord="tok")),
        patch("csuite.orchestrator.alert_tools.handle_create_alert", fake_alert),
    ):
...
def test_message_person_unknown_person_refused() -> None:
    with patch(
        "csuite.config.get_settings", return_value=_settings(discord="tok")
    ):
...
def test_message_person_sends_to_valid_negative_telegram_group_id() -> None:
    ...
    fake_tg = AsyncMock(return_value=None)
    with (
        patch("csuite.config.get_settings", return_value=_settings(telegram="tok")),
        patch("csuite.integrations.telegram_bot.send_message", fake_tg),
        patch("csuite.audit.log_event"),
        patch("csuite.orchestrator.schedule_tools._record_send_to_activity"),
    ):
...
def test_message_person_falls_back_across_channels_on_send_failure() -> None:
    ...
    with (
        ...
        patch("csuite.integrations.discord_bot.send_dm", failing_discord),
        patch("csuite.integrations.telegram_bot.send_message", ok_tg),
        patch("csuite.audit.log_event"),
        patch("csuite.orchestrator.schedule_tools._record_send_to_activity"),
    ):
        result = _call({"person_id": pid, "text": "hi"})
