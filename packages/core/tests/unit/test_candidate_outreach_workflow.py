...
from csuite.workflows.candidate_outreach import (
...
def _run(candidate_id: int, follow_up_count: int = 2) -> list:
    ...
    with patch(
        "csuite.workflows.candidate_outreach.route_to_specialist",
        mock_specialist,
    ), patch("csuite.workflows.candidate_outreach.retrieve", return_value=""):
