...
from csuite.orchestrator import outbound_guard, schedule_tools
from csuite.people.models import AvailabilityWindow, Person
...
def _use_settings(monkeypatch: pytest.MonkeyPatch, settings: _Settings) -> None:
    monkeypatch.setattr("csuite.config.get_settings", lambda: settings)
...
def _patch_person(monkeypatch: pytest.MonkeyPatch, person: Person) -> None:
    ...
    monkeypatch.setattr(
        "csuite.people.store.find_person_by_slack_id",
...
def test_fail_open_when_recent_sends_lookup_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "csuite.memory.episodic.recent_sends_for_channel_ref", _boom
    )
...
def test_fail_open_when_person_lookup_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("csuite.people.store.find_person_by_slack_id", _boom)
    assert outbound_guard.check_outbound_allowed("slack_dm", "U1", "hi") is None
...
def test_slack_handler_suppresses_without_sending(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "csuite.orchestrator.outbound_guard.check_outbound_allowed",
...
def test_telegram_handler_suppresses_without_sending(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("csuite.config.get_settings", lambda: _S())
    monkeypatch.setattr(
        "csuite.people.store.find_person_by_telegram_chat_id", lambda _ref: object()
    )
    monkeypatch.setattr(
        "csuite.orchestrator.outbound_guard.check_outbound_allowed",
    ...
    with patch("csuite.integrations.telegram_bot.send_message", new=send):
        result = asyncio.run(
...
def test_discord_handler_suppresses_without_sending(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("csuite.config.get_settings", lambda: _S())
    monkeypatch.setattr(
        "csuite.people.store.find_person_by_discord_id", lambda _ref: object()
    )
    monkeypatch.setattr(
        "csuite.orchestrator.outbound_guard.check_outbound_allowed",
    ...
    with patch("csuite.integrations.discord_bot.send_dm", new=send):
        result = asyncio.run(
