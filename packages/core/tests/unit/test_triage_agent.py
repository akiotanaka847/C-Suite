...
from csuite.agents.triage import TRIAGE_TOOL, TriageAgent, _parse_decision
...
def test_triage_model_follows_routing_model(monkeypatch) -> None:
    ...
    csuite.config at call time, so patch it there."""
    monkeypatch.setattr(
...
def test_triage_prompt_external_section_covers_every_registered_adapter() -> None:
    ...
    from csuite.monitoring.sources import _REGISTRY
    from csuite.prompts.triage_prompt import TRIAGE_PROMPT
