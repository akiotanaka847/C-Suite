...
from csuite.prompts.executive_persona import EXECUTIVE_PERSONA_PROMPT  # noqa: E402
...
def test_identity_addendum_uses_default_display_name() -> None:
    blocks = build_system_blocks()
    persona_text = blocks[0]["text"]
    assert "You are C-Suite" in persona_text
...
def test_identity_addendum_honors_exec_display_name_env(
    ...
    assert "You are C-Suite" not in persona_text
