...
from csuite.memory.decision_ledger import (
...
def _seed_companion_alert(db: Path, instance_id: int) -> None:
    ...
    from csuite.alerts.store import insert_alert
    insert_alert(
...
def client(db: Path) -> TestClient:
    ...
    from csuite.api.routes.decisions import router
    app = FastAPI()
...
def test_approve_creates_event(client: TestClient, db: Path) -> None:
    ...
    with patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=fake_gw):
...
def test_cancel_approved_event(client: TestClient, db: Path) -> None:
    ...
    import csuite.memory.episodic as _ep
...
def test_approve_clears_linked_alert(client: TestClient, db: Path) -> None:
    ...
    with patch("csuite.orchestrator.mcp_gateway.get_active_gateway", return_value=_approve_gateway()):
        res = client.post(f"/decisions/{iid}/approve", json={})
    assert res.status_code == 200
