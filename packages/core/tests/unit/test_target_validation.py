...
from csuite.monitoring import target_validation as tv
from csuite.monitoring.models import (
    PAGE_WATCH_FETCH_KEY,
    PAGE_WATCH_FETCH_XCRAWL,
)
from csuite.monitoring.sources import page_watch
from csuite.monitoring.sources._http import FetchOverflowError
...
async def test_ssrf_rejected_target_is_not_feed(
    ...
    monkeypatch.setattr(tv, "fetch_bounded", boom)
    monkeypatch.setattr(
        "csuite.integrations.xcrawl_client.scrape", boom_scrape,
    )
