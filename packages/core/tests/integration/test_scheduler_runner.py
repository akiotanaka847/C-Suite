...
from csuite.scheduler.runner import _execute_action, run_scheduler
...
def _stub_executive_deps(monkeypatch: pytest.MonkeyPatch, raise_in_chat: bool = False) -> None:
    ...
    monkeypatch.setattr(
        "csuite.orchestrator.executive.Executive", _StubExec
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "csuite.knowledge.retriever.retrieve", lambda query: ""
    )
...
def test_scheduler_holds_due_actions_when_no_company_profile(
    ...
    monkeypatch.setattr(
        "csuite.scheduler.runner._execute_action", _recording_execute
    )
